...
export default function ConversationItem(props: ConversationItemProps) {
    let { key, conersationName, conersationImages, active, haveStory, isOnline, isGroup, lastMessage } = props;
    return <>
        <li key={key}>
            {/* Chat user tab item */}
            <span className={`nav-link text-start ${active && "active"}`}>
                <div className="d-flex">
                    ...
                    <div className={`flex-shrink-0 avatar me-2 ${!isGroup && haveStory && "avatar-story"} ${!isGroup && isOnline && "status-online"}`}>

                        {isGroup ?
                            <GroupUser memberAvatarDetail={[
...
const GroupUser = (props: GroupUserInterface) => {
    ...
    return <ul className={`avatar-group avatar-group-${['one', 'two', 'three', 'four'][dAvatarCount - 1 < 4 ? dAvatarCount - 1 : 4]}`}>
        {memberAvatarDetail.slice(0,3).map((avatar,index) => <li key={index}className={`avatar avatar-${(dAvatarCount>3)?"xx":"x"}s`}> 
            <img
                className="avatar-img rounded-circle"
        ...
        {
            dAvatarCount>3 &&<li key={4} className={`avatar avatar-${(dAvatarCount>3)?"xx":"x"}s`}> 
            <img
                className="avatar-img rounded-circle"
